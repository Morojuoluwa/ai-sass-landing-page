...
const TestimonialColumn = (props:{className: string; testimonials: typeof testimonials; duration?:number}) =>(
  <div className={props.className}>
    <motion.div animate={{translateY:'-50%'}} transition={{repeat:Infinity, ease:"linear", repeatType:'loop', duration:props.duration|| 10}} className=" flex flex-col gap-6 pb-6 ">
      ...
      <React.Fragment key={index}>
        ...
        <div className="card" key={text}>
            ...
            <div className=" flex flex-col items-center gap-2 mt-5">
              <Image src={imageSrc} width={40} height={40} alt='' className="h-10 w-10 rounded-full"/>
              <div className=" flex flex-col">
                <h3 className=" font-medium tracking-tight leading-5">{name}</h3>
                <h3 className="tracking-tight leading-5">{username}</h3>
              </div>
            </div>
        </div>
      ))}
